...
某些引擎需要通过网络获取翻译结果，这需要借助 =pdd= 包的 =pdd-client= 组件进行网络处理。

默认情况下网络请求是通过 =pdd-url-client= 处理的，它的底层是 url.el，效率很低。

组件 =pdd-plz-client= 实现了通过 curl 进行 http 请求的目的，效果好很多很多。

可以通过 =gt-default-http-client= 对请求客户端进行配置。当然，如果检测到系统内已经存在 curl 程序并安装了 [[https://github.com/alphapapa/plz.el][plz]] 包，组件 =pdd-plz-client= 将自动设定为默认请求器。
...
#+begin_src emacs-lisp
  (setq gt-default-http-client
        (lambda (url)
          (if (string-match-p "google\\|deepl\\|openai" url)
              (pdd-plz-client :args '("--proxy" "socks5://127.0.0.1:9999"))
            (pdd-plz-client))))

  (setq gt-default-http-client
        (lambda (url)
          (let ((proxy? (string-match-p "google\\|deepl\\|openai" url)))
