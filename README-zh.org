...
网络请求功能是借助 [[https://github.com/lorniu/pdd.el][pdd.el]] 包实现的。pdd 使用外部程序 =curl= 或内置的 =url.el= 进行请求。

如果想使用 =curl= 的方式，只需要在系统中安装 curl 和 [[https://github.com/alphapapa/plz.el][plz]] 包。pdd 将会优先使用它们。

当然，你也可以通过 =gt-http-backend= 设定使用哪种方式，并通过 =gt-http-proxy= 来设定代理:
#+begin_src emacs-lisp
  ;; 显式指定使用的客户端
  (setq gt-http-backend (pdd-url-backend))  ; 基于 url.el 的
