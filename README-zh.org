...
作为一个翻译框架，它有很多优点:
- 支持多种翻译引擎，比如 Bing, Google, DeepL, 有道词典, StarDict 等
...
#+begin_src emacs-lisp
  ...
  ;; 上述配置的意思是，初始化默认翻译器，让其在 en 和 zh 之间通过 Google 进行翻译，
  ;; 结果将显示在 Echo Area 中
#+end_src
...
#+begin_src emacs-lisp
  (setq gt-default-translator
        (gt-translator
         ...
         :engines (list (gt-bing-engine) (gt-google-engine)) ; 指定多引擎
         :render  (gt-buffer-render)))                       ; 配置渲染器

  ;; 上述配置的意思是，初始化默认翻译器，让其将当前 buffer 中的所有段落交给 DeepL 和 Google 进行翻译，
...
内置的 Engine 实现有:
- =gt-engine-deepl=, DeepL 翻译
- =gt-engine-bing=, 微软翻译
- =gt-engine-google/gt-engine-google-rpc=, Google 翻译
- =gt-engine-youdao=, 有道翻译
- =gt-engine-stardict=, StarDict，离线翻译
...
我现在使用翻译不多，这个框架纯粹是兴趣使然。因为对翻译工作的认知有限，某些功能设置未必合理。
