...
如果只想让特定请求走代理，可以仿照下面方式设置:
#+begin_src emacs-lisp
  (setq my-client-1 (gt-url-http-client))
  (setq my-client-2 (gt-plz-http-client))
  (setq my-client-3 (gt-plz-http-client :args '("--proxy" "socks5://127.0.0.1:9999")))

  (setq gt-default-http-client
        (lambda (host)
          (cond ((not (require 'plz nil t)) my-client-1)
                ((string-match-p "google\\|deepl\\|openai" host) my-client-3)
                (t my-client-2))))
#+end_src
