...
*** pdd-url-client/pdd-plz-client (gt-default-http-client)

Some engines need to fetch translation results over the network, which requires network processing with the help of the =pdd-client= component of package =pdd=.

By default, =pdd-url-client= is used as the http client, which is inefficient.

The component =pdd-plz-client= uses =curl= to send the request, which is much better.

Config =gt-default-http-client= to switch http client. Or just make sure =curl= and [[https://github.com/alphapapa/plz.el][plz]] is exists in your system, then =pdd-plz-client=
...
#+begin_src emacs-lisp
  ...
  (setq gt-default-http-client
        (pdd-url-client :proxies '(("http" . "host:9999") ("https" . "host:9999"))))

  ;; for pdd-plz-client
  (setq gt-default-http-client
        (pdd-plz-client :args '("--proxy" "socks5://127.0.0.1:9999")))
