...
The network request is sent with the help of the [[https://github.com/lorniu/pdd.el][pdd.el]] package. pdd uses the external program =curl= or the built-in =url.el= to send requests.

If you prefer the =curl= style, just ensure =curl= and package [[https://github.com/alphapapa/plz.el][plz]] are on your system. pdd will use them automatically.

Of course, you can specify with =gt-http-backend=, and set the proxy with =gt-http-proxy=:
#+begin_src emacs-lisp
  ;; specify client explicitly
  (setq gt-http-backend (pdd-curl-backend)) ; base on curl
  (setq gt-http-backend (pdd-url-backend))  ; base on url.el
